...
To avoid the construction of a pressure space $\mathcal{P}_{h,k}$ such that it contains both piecewise constant and piecewise zero-mean functions, two new variables can be added to the problem, namely the elemental average pressure $\bar{p}_h \in \bar{\mathcal{P}}_h$ and the distributed force $g_h \in \bar{\mathcal{P}}_{h}$, allowing not only the construction of $\mathcal{P}_{h,k}$ using traditional piecewise polynomials of order $k$ but also the condensation of all the higher-order pressure functions. It is worth mentioning that for compressible solids, the pressures are computed explicitly from the displacements, so these two new spaces are no longer necessary.
