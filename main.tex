...
For simplicity, let us consider a pure homogeneous Dirichlet problem, i.e. $\bm{u}_D=\bm{0}$ on $\partial\Omega$. The solution $\{\bm{u}_0,p_0,\bm{t}^t_0 \} \in \{\mathcal{V}_0 \times L^2 \times \mathcal{L}^t_0 \}$ is assumed to satisfy the semi-hybrid weak form of Eq. \eqref{eq:semi-hybrid-weak}. A locally equilibrated stress solution can be retrieved by relaxing the inter-element continuity requirement of the normal component of the displacement field. In the standard hybridization fashion, normal tractions $\bm{t}^n \in \mathcal{L}^n(\Gamma_h,\mathbb{R}^d)$ are added to the mesh skeleton. The continuity of the normal component of the displacement field can be imposed in a weak manner by setting $\langle\bm{t}^n,\bm{v}^n\rangle_{\Gamma_h} = 0$, that is,
\begin{equation*}
	\sum_{E \in \Gamma_h} \int_{E} \jump{\bm{v}\cdot\bm{n}} \bm{t}^n \,d\partial K = 0 .
\end{equation*}

Therefore, we can search for $\{\bm{u},p,\bm{t}^t,\bm{t}^n\} \in \bar{H}_0(\text{div}) \times L^2 \times \mathcal{L}^t_0 \times \mathcal{L}^n_0$ such that:
...
The solution $\{\bm{u}_0,p_0,\bm{t}^t_0\}$ satisfies the Eqs. \eqref{eq:semi-hybrid-weak-broken-b} and \eqref{eq:semi-hybrid-weak-broken-c}. Recalling that $\bm{u}_0 \in \mathcal{V}_0 \subset H_0(\text{div})$ implies $\jump{\bm{u}\cdot\bm{n}}_E=0$, Eq. \eqref{eq:semi-hybrid-weak-broken-d} is also automatically satisfied. Rewritting Eq. \eqref{eq:semi-hybrid-weak-broken-a} in a residual form yields:
...
\noindent where the right-hand side of Eq. \eqref{eq:semi-hybrid-residual} is not null since $\bm{v} \in \bar{H}_0(\text{div})$. If the approximation spaces are chosen properly, the number of functions in $\mathcal{L}^n_0$ matches the ones in $\bar{H}_0(\text{div})$, so that the term $\langle\bm{t}^n,\bm{v}\rangle_{\Gamma_h}$ gives rise to an invertible matrix where the solution $\bm{t}^n$ can be computed. The locally equilibrated stress solution is then obtained for each $\partial K$ as $\bm{t} = \bm{t}^t + \bm{t}^n$.
