...
\newcommand{\carlos}{\color{blue}{\bf\Large CP} \color{cyan} }
...
The Cook's membrane is a classical benchmark for the study of locking phenomena of incompressible or nearly-incompressible elasticity problems under shear and bending conditions. It consists of a tapered cantilever beam clamped at its left side and subjected to a shear load in the $y$ direction at the right end, according to Figure \ref{fig:cooks-geometry}. The material is assumed to have a linear behaviour with a constant Young modulus $E=240.565$ MPa. Three different Poisson coefficients were tested: $\nu=0.3333$, $\nu=0.4999$ and $\nu=0.5$. This problem can be seen in many books and papers, such as in \cite{cook1989concepts,elguedj2008b}, with different material properties, geometries and boundary conditions. Some works also present a geometric non-linear analysis, under elastic and elastoplastic regimes. In this work, we only focus on the linear elastic regime.
