...
The three dimensional case is studied using similar meshes with hexahedra and tetrahedra elements for $\nu=0.5$. Zero normal displacement is imposed at the facets parallel to the $x-y$ plane to simulate a plane strain condition. Table \ref{tab:cooks-3d} shows the displacement obtained with the different partitions and polynomial degrees. The results are in agreement with the two-dimensional case. Lastly, we plot the Von mises stress over the deformed configuration using the finest meshes showing that a smooth field is obtained with the DHM-H(div) formulation, as depicted in Figure \ref{fig:cooks-3d-stress}.
