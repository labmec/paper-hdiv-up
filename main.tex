...
\noindent for which $\langle\bm{t},\bm{v}\rangle_{\Gamma_h}=\langle\bm{t}^n,\bm{v}\rangle_{\Gamma_h}+\langle\bm{t}^t,\bm{v}\rangle_{\Gamma_h}=\langle\bm{t}^t,\bm{v}\rangle_{\Gamma_h}$ holds $\forall \bm{v} \in \mathcal{V}$. The equivalent dual norm restricted to the tangential traction space can be analogously written as $\lVert\bm{t}^t\rVert_{\mathcal{L}}=\sup_{\bm{v}\in\mathcal{V}}\frac{\langle\bm{t}^t,\bm{v} \rangle_{\Gamma_h}}{\lVert\bm{v}\rVert_{H^1,\mathcal{T}_h}}$.
