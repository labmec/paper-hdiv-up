...
{\giovane escrever algo sobre o que e conservacao local em termos de tensoes.}

Consider a pure homogeneous Dirichlet problem, i.e. $\bm{u}_D=\bm{0}$ on $\partial\Omega$. Let $\{\bm{u}_0,p_0,\bm{t}^t_0 \} \in \{\mathcal{V}_0 \times L^2 \times \mathcal{L}^t_0 \}$ be the solutions of the semi-hybrid weak form of Eq. \eqref{eq:semi-hybrid-weak}. A locally equilibrated stress solution can be recovered by relaxing the inter-element continuity requirement {\nathan is it a requirement or a consequence of the approximation space choice? Maybe we can just say we relax that continuity without talking about requirement or consequence} of the normal component of the displacement field. Following the standard hybridization approach, normal tractions $\bm{t}^n \in \mathcal{L}^n(\Gamma_h,\mathbb{R}^d)$ are introduced to the mesh skeleton {\nathan this previous phrase sounds weird. What about cutting it and just saying later that $\bm{t}^n$ is the normal traction?}. The continuity of the normal component of the displacement field can be enforced weakly by imposing $\langle\bm{t}^n,\bm{v}^n\rangle_{\Gamma_h} = 0$, that is,
\begin{equation*}
	\sum_{E \in \Gamma_h} \int_{E} \jump{\bm{v}\cdot\bm{n}} \bm{t}^n \,d\partial K = 0 .
\end{equation*}

Thus, in this newly defined broken space, we seek $\{\bm{u},p,\bm{t}^t,\bm{t}^n\} \in \bar{H}_0(\text{div}) \times L^2 \times \mathcal{L}^t_0 \times \mathcal{L}^n_0$ such that {\nathan Is $\bar{H}_0(\text{div})$ the broken Hdiv space? Is so, we should define it.}:
\begin{subequations} \label{eq:semi-hybrid-weak-broken}
	\begin{align}
		a\left(\bm{u},\bm{v}\right)_{\mathcal{T}_h} - b\left( p, \bm{v}\right)_{\mathcal{T}_h} + \langle\bm{t}^t,\bm{v}\rangle_{\Gamma_h} + \langle\bm{t}^n,\bm{v}\rangle_{\Gamma_h} &= \left(\bm{b},\bm{v}\right)_{\mathcal{T}_h} ~~\forall\, \bm{v} \in \bar{H}_0(\text{div}),\label{eq:semi-hybrid-weak-broken-a}\\ 
		-b\left(\bm{u}, q\right)_{\mathcal{T}_h} - c\left(p,q \right)_{\mathcal{T}_h} &= 0 ~~\forall\, q \in L^2, \label{eq:semi-hybrid-weak-broken-b}\\
...
\noindent where the right-hand side of Eq. \eqref{eq:semi-hybrid-residual} is not null since $\bm{v} \in \bar{H}_0(\text{div})$. If the approximation spaces are chosen properly {\nathan properly sounds vague here}, the number of functions in $\mathcal{L}^n_0$ matches the ones in $\bar{H}_0(\text{div})$, so that the term $\langle\bm{t}^n,\bm{v}\rangle_{\Gamma_h}$ gives rise to an invertible matrix where the solution $\bm{t}^n$ can be computed. The locally equilibrated stress solution is then obtained for each $\partial K$ as $\bm{t} = \bm{t}^t + \bm{t}^n$. {\nathan It feels to me that this section is lacking a small conclusion. Like, why does it matter that we can recover a locally equilibrated stress solution?}
