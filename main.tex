...
\end{equation*}
{\nathan I am not sure you can just say $\bm{v} \in L^2(\Omega,\mathbb{R}^d)$. You might need $\bm{v} \in [L^2(\Omega,\mathbb{R}^d)]^d$}

\noindent equipped with the norm $\| \cdot \|_{H(\text{div})}$ and \(L^2(\Omega,\mathbb{R}^d)=\{\bm{v} : v_i \in L^2(\Omega) : i \in\{2,3\} \}\). 
...
\end{equation*}
{\nathan the above equations looks weird. Is v the trace of u?}
\begin{equation*}
...
\noindent with $\kappa$ standing for the bulk modulus, which for the generalized tridimensional case, is computed as $\kappa=\frac{2\mu+3\lambda}{3}$. Its value for the plane stress and plane strain hypothesis can also be derived by imposing the constraints on each case {\giovane should we put the derivation of the bulk modulus for the plane analyses in an appendix?} {\nathan I am not sure, I feel these are stanrdard and known coefficients}. Plugging Eqs. \eqref{eq:pressure} and \eqref{eq:hook} into Eq. \eqref{eq:stress-decomposition}, the deviatoric stress tensor $\boldsymbol{\sigma}'$ is computed as:
...
The mixed problem of Eq. \eqref{eq:weak-mixed} can be presented in an alternative way by relaxing the continuity requirement of the displacement functions across elements. {\nathan\sout{Let d}Define the space of the stress traces over the mesh skeleton as}
\begin{equation}
...
\end{equation}
{\nathan check if $\bm{v} \in L^2(\Omega)$ should be $\bm{v} \in [L^2(\Omega)]^d$}
...
\end{equation}
{\nathan check if $\bm{t} \in L^2(\Omega)$ should be $\bm{t} \in [L^2(\Omega)]^d$}
...
According to the Brezzi's theory {\nathan theorem?} in \cite{girault2012finite,brezzi2012mixed,arnold1988new}, the FE spaces associated to each variable cannot be chosen arbitrarily. 

\sout{Let assume the discontinuous pressure space} {\nathan Assume the pressure space is discontinuous, i.e.:}
\begin{equation}
...
\begin{equation*}
	\text{rank}~ \bm{E}^T = \text{dim} ~\mathcal{W}_{h,k},
\end{equation*}

\noindent with $\bm{E}^T=[\langle\bm{z},\bm{s}^t\rangle_{\Gamma_h}]$. In practical terms, the trace compatibility is guaranteed by enriching the internal functions of $\mathcal{V}_{h,k}$, which shall be demonstrated numerically.
...
Solving the problem \eqref{eq:double-hybrid-discrete} in the way it is presented would demand a lot of computational efforts when comparing to traditional $H^1$ primal methods or even with mixed Taylor-Hood approximations. First, the divergence-compatible pair $\mathcal{V}_{h,k} \times \mathcal{P}_{h,k}$ includes a larger number of functions for a given polynomial order $k$ than the traditional $H^1$ subspaces. Second, the introduction of Lagrange multipliers due to the hybridizations not only increase the number of unknowns but also the system to be solved results in a more challenging saddle-point problem. Moreover, the advantages of the DHM-H(div) formulation only appear after applying the static condensation procedure, as shall be verified next.

Hybridization techniques have been widely used in the literature along with static condensation to reduce the size of global system to be solved. The procedure consists on separating the global degrees of freedom (DOFs) into two sets: one containing the DOFs that are possible to be solved within a single element, called internal DOFs, and the other containing the DOFs that are shared between elements and therefore cannot be solved locally, called external (or main) DOFs.

As previously mentioned, the displacement functions $\bm{u}_h \in \mathcal{V}_{h,k}$ can be separated into divergence-free functions with vanishing normal components over the element boundaries, namely $\mathring{\bm{u}}_h$, and the remaining functions with continuous normal components across elements boundaries, $\bm{u}^\partial_h$. The divergence-free functions can be classified as internal functions because they do not contribute to the pure dilation and compression deformation modes that could affect multiple elements.

To avoid the construction of a pressure space $\mathcal{P}_{h,k}$ such that it contains both piece-wise constant and piece-wise zero-mean functions, two new variables can be added to the problem, namely the elemental average pressure $\bar{p}_h \in \bar{\mathcal{P}}_h$ and the distributed force $g_h \in \bar{\mathcal{P}}_{h}$, allowing not only the construction of $\mathcal{P}_{h,k}$ using traditional piece-wise polynomials of order $k$ but also the condensation of all the higher-order pressure functions. It is worth mentioning that for compressible solids, the pressures are computed explicitly from the displacements, so these two new spaces are no longer necessary.

Unden these assumptions, the DHM-H(div)($\mathcal{S}_{h,k}$) can be written as: find $\{\bm{u}^\partial_h,\mathring{\bm{u}}_h,p_h,\bm{t}^t_h,\bm{w}_h,\bar{p}_h,\bar{g}_h\} \in \{\mathcal{V}^\partial_h \times \mathring{\mathcal{V}}_h \times \mathcal{P}_h \times \bar{\mathcal{L}}_h \times \mathcal{W}_h \times \bar{\mathcal{P}} \times \bar{\mathcal{P}} \}$ such that:
\begin{subequations} \label{eq:double-hybrid-six-field}
	\begin{align}
		a\left(\bm{u}^\partial_h,\bm{v}\right)_{\mathcal{T}_h} + a\left(\mathring{\bm{u}}_h,\bm{v}\right)_{\mathcal{T}_h} - b\left( p_h, \bm{v}\right)_{\mathcal{T}_h} +\langle\bm{t}^t_h,\bm{v}\rangle_{\partial\mathcal{T}_h} &= \left(\bm{b},\bm{v}\right)_{\mathcal{T}_h} + \langle\bm{t}_N\cdot\bm{n},\bm{v}\cdot\bm{n}\rangle_{\partial\Omega_N} ~~\forall\, \bm{v} \in \mathcal{V}_{h,k},\label{eq:double-hybrid-six-field-a}\\ 
		-b\left(\bm{u}^\partial_h, q\right)_{\mathcal{T}_h} - b\left(\mathring{\bm{u}}_h, q\right)_{\mathcal{T}_h} - c\left(p_h,q \right)_{\mathcal{T}_h} &= 0 ~~\forall\, q \in \mathcal{P}_{h,k}, \label{eq:double-hybrid-six-field-b}\\
		\langle\bm{u}^\partial_h,\bm{s}^t\rangle_{\partial\mathcal{T}_h} + \langle\mathring{\bm{u}}_h,\bm{s}^t\rangle_{\partial\mathcal{T}_h} - \langle\bm{w}_h,\bm{s}^t\rangle_{\Gamma_h} &= 0 ~~\forall\, \bm{s}^t \in \bar{\mathcal{L}}^t_{h,k}, \label{eq:double-hybrid-six-field-c}\\
		-\langle\bm{t}^t_h,\bm{z}^t\rangle_{\Gamma_h} &= -\langle\bm{t}_N^t,\bm{z}^t\rangle_{\partial\Omega_N} ~~\forall\, \bm{z}^t \in \mathcal{W
		}_{h,k}, \label{eq:double-hybrid-six-field-d}\\
		\langle\bm{u}^\partial_h,\bm{v}\rangle_{\partial\mathcal{T}_h} + \langle\mathring{\bm{u}}_h,\bm{v}\rangle_{\partial\mathcal{T}_h} - \langle\bm{w}_h,\bm{v}\rangle_{\Gamma_h} &= 0 ~~\forall\, \bm{v} \in \mathcal{V}_{h,k}, \label{eq:double-hybrid-six-field-e}\\
		\langle p_h, q \rangle_{\mathcal{T}_h} - \langle \bar{p}_h, q \rangle_{\mathcal{T}_h} &= 0 ~~\forall\, q \in \bar{\mathcal{P}}, \label{eq:double-hybrid-six-field-f}\\
		\langle \bar{g}_h, q \rangle_{\mathcal{T}_h} &= \langle \bm{b}, \bm{v} \rangle_{\mathcal{T}_h} + \langle \bm{t}_N\cdot\bm{n}, \bm{v}\cdot\bm{n} \rangle_{\partial\Omega_N} ~~\forall\, q \in \bar{\mathcal{P}}. \label{eq:double-hybrid-six-field-g}
	\end{align}
\end{subequations}
{\giovane Parei aqui}
Recalling that $\bm{t}^t \in \bar{\mathcal{V}}_{h,k}$ is discontinuous over the mesh sleleton, the problem of Eq. \eqref{} can be rearranged in a matricial form as:




For a compressible solid, a symmetric positive-definite matrix is obtained with two unknowns, namely $\mathbf{u} \in H(\text{div},\Omega)$ and $\mathbf{u}^t \in H^{1/2}(\partial\Omega_e)$. For the incompressible case where pressures cannot be computed explicitly from the displacements, one mean pressure per element acts as a Lagrange multiplier in the global system. This still gives rise to a saddle-point matrix, however, with much better spectral properties and easier to solve compared to Eqs. \eqref{eq:semihybrid1}-\eqref{eq:semihybrid3}.
