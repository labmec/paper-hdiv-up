...
Then, the tangential tractions $\bm{t}^t$ in Eq. \eqref{eq:block-matrix-1} can be condensed onto the tangential velocities $\bm{w}$, yielding
...
\noindent where $\bm{A}_{\smallcirc\smallcirc}^*=\bm{L}\bar{\bm{A}}_{\smallcirc\smallcirc}^{-1}\bm{L}^T$ and $\bm{A}_{\partial\smallcirc}^*=\bm{L}\bar{\bm{A}}_{\smallcirc\smallcirc}^{-1}\bar{\bm{D}}$. The matrix $\bm{A}_{\smallcirc\smallcirc}$ is symmetric positive definite due to the number of functions that approximate the tangential displacements within the block $\bm{L}$ being exactly the same as the number of tangential traction functions \cite{puga2025stable}. The static condensation can be further proceeded once more to eliminate $p \text{ and } \bar{g}$, giving rise to the following matrix structure:
...
As a consequence of this procedure, a symmetric positive-definite matrix is obtained for a compressible solid with two unknowns, namely $\{\bm{u}^\partial_h,\bm{w}\} \in \mathcal{V}^\partial \times \mathcal{W}$. In this case, the block matrix $\bm{C}$ is positive definite, hence it can be condensed without the need of adding $\bar{p}$ and $\bar{g}$. For the incompressible case where pressures cannot be computed explicitly from the displacements, one mean pressure per element $\bar{p}_h \in \bar{\mathcal{P}}$ acts as a Lagrange multiplier in the global system. This still gives rise to a saddle-point matrix, however, with much better spectral properties and easier to solve compared to Eqs. \eqref{eq:semi-hybrid-weak-a}-\eqref{eq:semi-hybrid-weak-c}.
